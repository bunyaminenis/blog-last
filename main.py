...
class MakePostForm(FlaskForm):
    ...
    url_for_bck_img = StringField(label="Blog Image URL", validators=[DataRequired()])
...
def add_new_post():
    ...
    if make_post_form.validate_on_submit():
        new_blog = BlogPost(title=make_post_form.blog_post_title.data,
                            date= date.today().strftime("%B %#d, %Y"),
                            body=make_post_form.body.data,
                            author=make_post_form.author_name.data,
                            img_url=make_post_form.url_for_bck_img.data,
...
def edit_post(post_id):
    ...
    edit_post_form = MakePostForm(
        blog_post_title=post.title,
        subtitle=post.subtitle,
        author_name=post.author,
        url_for_bck_img=post.img_url,
        body=post.body,
    )

    if edit_post_form.validate_on_submit():
        post.title = edit_post_form.blog_post_title.data
        post.subtitle = edit_post_form.subtitle.data
        post.author = edit_post_form.author_name.data
        post.img_url = edit_post_form.url_for_bck_img.data
